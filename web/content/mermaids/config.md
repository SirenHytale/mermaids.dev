...
> **New in 3.2.0:** you no longer have to edit these files by hand at all -- see the [in-game Server Configuration menu](#in-game-server-configuration-menu) below. The Mermaids Config also gains [Tail-Color-Permissions](/mermaids/permissions/#tail-color-permissions), and the [Drying Out](#drying-out) damage defaults were **lowered** -- see that section for exactly when your existing values are and aren't touched.
